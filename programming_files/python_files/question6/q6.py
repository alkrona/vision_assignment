...
from python_files.lecture_function import rgb_to_bandw
from matplotlib import pyplot as plt
logging.basicConfig(level=logging.INFO,filename="information_q6.log")
import pandas as pd
import seaborn as sns
from trial import find_parent
def main():
    file_name = "../../../image_data/WhiteMixComplex2019.tif"
    ...
    blobdata=correcting_blobs(blobdata,array)
    logging.info(f"this is the useful array{array}")
    print(f"this is the useful array{array}")
...
def postprocessing(labeled_array:np.array)->np.array:
    ...
    plt.show()
def correcting_blobs(blob_data:np.array,eq_list:list)->np.array:
    ...
    print(eq_list)
    for i in range(blob_data.shape[0]):
        for j in range(blob_data.shape[1]):
            blob_data[i,j]=eq_list[blob_data[i,j]]
    return blob_data
        ...
        for j in range(1,height-1):
            ...
            if blob_data[i,j]==255:
                ...
                if blob_data[i,j-1] ==0 and blob_data[i-1,j-1]==0 and blob_data[i-1,j]==0 and blob_data[i-1,j+1]==0:
                    blob_data[i,j]=count
                    print(f" printing the occurances of count{count} and eq list is {eq_list}")
